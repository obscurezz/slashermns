...
function getNumberText(value, percent)
	return string.format(percent and "%d%%" or "%d", math.floor(value))
end
...
function getModifiedNumberForArmor(value, base, percent, negative, full)
	local bonus = value - base
	if bonus == 0 then
		return getNumberText(base, percent)
	end
...
function getAttackDurationText(attack)
	...
	if
		...
		result = getModifiedStringText(attack.infinite and getInterfaceText("X015TA0019") or getInterfaceText("X015TA0020"), modified)
	elseif
		attackType == Attack.BoostDamage or
		attackType == Attack.LowerDamage or
		attackType == Attack.LowerInitiative then
		result = getModifiedStringText(attack.infinite and getInterfaceText("X015TA0021") or getInterfaceText("X015TA0020"), modified)
	elseif attackType == Attack.TransformOther then
		result = getModifiedStringText(attack.infinite and getInterfaceText("X015TA0019") or getInterfaceText("X015TA0021"), modified)
	else
		result = getInterfaceText("X015TA0018")
	end
	return result
end
...
function getAttackDrainText(unitImpl, attack, boostDamageLevel, lowerDamageLevel)
	...
	if attackType == Attack.DrainOverflow then
		...
		return replace(replace("%ATTACK% (%OVERFLOW%)", "%ATTACK%", result), "%OVERFLOW%", getModifiedStringText(getInterfaceText("X005TA0801"), attackType ~= generated.type))
	else
		return result
	end
end
...
function getHp2Field(unit, unitImpl)
	...
	local bonus = unit.hpMax - unitImpl.generated.hp
	local result
	if bonus ~= 0 then
		result = "("..getBonusNumberText(bonus, false, false)..")"
	else
		result = ""
	end
	return replace(replace("%MAXHP% %BONUS%", "%MAXHP%", getNumberText(unit.hpMax, false)), "%BONUS%", result)
end
...
function getArmorField(unitImpl, isInBattle, fortificationArmor, shatteredArmor)
	...
	return getModifiedNumberForArmor(unitArmor, unitImpl.generated.armor, false)
end
...
function getEffhpField(unit, unitImpl, isInBattle, fortificationArmor, shatteredArmor)
	...
	if armor >= 100 then
		return replace(getInterfaceText("X070TA1024"), "%HP%", getNumberText(2147483647, false)) -- max int32
	end

	local hp = unit and unit.hp or unitImpl.hp
	return replace(getInterfaceText("X070TA1024"), "%HP%", getNumberText(math.floor(hp * 100 / (100 - armor)), false))
end

function getRegenField(unitImpl, unitRegen)
	return replace(getInterfaceText("X150TA1003"), "%REG%", getModifiedNumberText(unitRegen, unitImpl.generated.regen, true))
end

function getXpKillField(unitImpl)
	return replace(getInterfaceText("X150TA1004"), "%XPKILL%", getModifiedNumberText(unitImpl.xpKilled, unitImpl.generated.xpKilled, false, true))
end

function getMagResField(unit)
	local result
	if unit == nil then
		result = 0
	else
...
function getEffBuffField(unit)
	...
	if unit == nil then
		...
		result = getGlobalBuffEffect(unit)
	end
	return replace(getInterfaceText("X150TA1008"), "%BUFF%", getNumberText(100 + result * 100, true))
end

function getEffDebuffField(unit)
	local result
	if unit == nil then
		result = 0
	else
		result = getGlobalDebuffEffect(unit)
	end
	return replace(getInterfaceText("X150TA1009"), "%DEBUFF%", getNumberText(100 + result * 100, true))
end

function getApField(unitImpl)
	-- if unit then
	-- 	return ""
	-- end
...
function getNegotiationField(unitImpl)
	-- \hL;\s120;\fmedbold;Íåïîäêóïíîñòü:\fnormal; %NG%
	local base = unitImpl.generated.negotiate + 10
	local result = getInterfaceText("X150TA1002")
	return replace(result, "%NG%", getModifiedNumberText(unitImpl.negotiate + 10, base, true))
end

function getInstructorField(unitImpl)
	local result = getInterfaceText("X150TA1007")
	return replace(result, "%INS%", getNumberText(unitImpl.instructor, false))
end
...
function getAltAttackField(unitImpl)
	...
	-- "%ATTACK% or %BLANK%"
	return replace(replace(getInterfaceText("X005TA0829"), "%ATTACK%", getAttackNameText(unitImpl.attack1)), "%BLANK%", "")
end
...
function getAttackField(unitImpl)
	...
	if not attackHasInfinite(attack1) then
		return getAttackNameText(attack1)
	else
		return getAttackNameText(attack1) .. " (" .. getAttackDurationText(attack1) .. ")"
	end
end
...
function getSecondField(unitImpl)
	...
	if not attackHasInfinite(unitImpl.attack2) then
		-- " / %ATTACK%"
		return replace(getInterfaceText("X005TA0785"), "%ATTACK%", getAttackNameText(unitImpl.attack2))
	else
		return replace(getInterfaceText("X005TA0785"), "%ATTACK%", getAttackNameText(unitImpl.attack2)) .. " (" .. getAttackDurationText(unitImpl.attack2) .. ")"
	end
end
...
function getHitField(unitImpl)
	...
	return getAttackPowerText(unitImpl, attack1)
end
...
function getEffectField(unitImpl)
	...
	if
		attackType == Attack.Heal or
		attackType == Attack.BestowWards then
		effect = getInterfaceText("X005TA0504") -- "Heal"
	elseif attackType == Attack.BoostDamage then
		effect =  getInterfaceText("X005TA0534") -- "Boost"
	elseif attackType == Attack.LowerDamage then
		effect = getInterfaceText("X005TA0547") -- "Lower"
	elseif attackType == Attack.LowerInitiative then
		effect = getInterfaceText("X005TA0551") -- "Lower initiative"
	else
		effect = getInterfaceText("X005TA0503") -- "Damage"
	end
	return effect	
end
...
function getDurationField(unitImpl)
	...
	local duration = getAttackDurationText(attack1)
	...
	return replace(getInterfaceText("X005TA0137"), "%DURATION%", duration)
end
...
function getDamageField(unitImpl, boostDamageLevel, lowerDamageLevel)
	...
	local maxTargets = attack1.maxTargets
	local result = getAttackDamageText(unitImpl, attack1, boostDamageLevel, lowerDamageLevel, maxTargets)
	if unitImpl.attack2 then
...
function getDrainField(unit, unitImpl, boostDamageLevel, lowerDamageLevel)
	...
	-- Replace hard-coded string with getInterfaceText of your drainDescription from textids.lua
	return replace(replace(getInterfaceText("X015TA0008"), "%DRAINEFFECT%", getInterfaceText("X015TA0009")), "%DRAIN%", drain)
end
...
function getSourceField(unitImpl)
	...
	local result = getAttackSourceText(attack1)
	-- if unitImpl.altAttack then
	-- 	-- "%ATTACK% or %BLANK%"
	-- 	local nonAltAttack = getAttackSourceText(unitImpl.altAttack)
	-- 	result = replace(replace(getInterfaceText("X005TA0829"), "%ATTACK%", getAttackSourceText(unitImpl.attack1)), "%BLANK%", nonAltAttack)
	-- end
...
function doGetTxtStats2Text(unit, unitImpl, isInBattle, fortificationArmor, shatteredArmor, unitRegen, pattern)
	-- \c000;000;000;%HP%%REG%%XPKILL%%MAGRES%
	local text = getInterfaceText("X150TA1006")
	if not unit then
		-- Only add dynamic upgrade values for unit-type encyclopedia (to avoid screen cluttering)
		text = getTxtStats2WithDynUpgradeText(text, unitImpl)
	end
	text = replace(text, "%REG%", getRegenField(unitImpl, unitRegen))
	text = replace(text, "%XPKILL%", getXpKillField(unitImpl))
	text = replace(text, "%HP%", getEffhpField(unit, unitImpl, isInBattle, fortificationArmor, shatteredArmor))	
	text = replace(text, "%MAGRES%", getMagResField(unit))
	text = replace(text, "%BUFF%", getEffBuffField(unit))
	text = replace(text, "%DEBUFF%", getEffDebuffField(unit))
	return text
end

function doGetTxtLeaderInfoText(unit, unitImpl)
	-- \c000;000;000;%LDRSHP%%AP%%NBBAT%%NG%
	local text = getInterfaceText("X005TA0809")
	text = replace(text, "%AP%", getApField(unitImpl))
	text = replace(text, "%LDRSHP%", getLdrshpField(unitImpl))
	text = replace(text, "%NBBAT%", getNbbatField(unit))
	text = replace(text, "%NG%", getNegotiationField(unitImpl))
	-- text = replace(text, "%INS%", getInstructorField(unitImpl))
	return text
end
...
function doGetTxtAttackInfoText(unit, unitImpl, boostDamageLevel, lowerDamageLevel, lowerInitiativeLevel)
	...
	text = replace(text, "%DRAIN%", getDrainField(unit, unitImpl, boostDamageLevel, lowerDamageLevel))
