...
function getAttackDamage(unit, prev)
	...
	if _GroupInfo_UnitModifierAmount(umods, black_knight) > 0 then
		...
		for i = 1, #nearUnits do
			...
			if _GroupInfo_UnitModifierAmount(mods, death_knight) > 0 then
	            		result = svMultimplyDamage1(unit, prev, 0.15)
        	    		break
            		end
        	end	
	end
	if _GroupInfo_UnitModifierAmount(umods, death_knight) > 0 then
		local nearUnits = _GroupInfo_getLeftAndRightNearestUnits(unit, true)
		local mods
		for i = 1, #nearUnits do
			mods = _GroupInfo_UnitModifiers(nearUnits[i])
			if _GroupInfo_UnitModifierAmount(mods, black_knight) > 0 then
	            		result = svMultimplyDamage1(unit, prev, 0.15)
        	    		break
            		end
        	end	
