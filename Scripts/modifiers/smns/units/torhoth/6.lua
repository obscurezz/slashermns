...
function getAttackPower(unit, prev)
   return svMultimplicativeEffectPower1(unit, prev, 1.30)
end

function getAttackDamage(unit, prev)
   return svMultimplicativeEffectDamage1(unit, prev, 1.30)
end
...
function getAttack2Power(unit, prev)
   ...
   if unit.impl.attack2 ~= nil then
      ...
      res = svMultimplicativeEffectPower2(unit, prev, 1.3)
...
function getAttack2Damage(unit, prev)
   if statsCheck_isDirectDmgType(unit.impl.attack2.type) then 
      return svMultimplicativeEffectDamage2(unit, prev, 1.3)
   end
   return prev
end