...
-- myModValue = Id.new('g070um0271').value
...
function canApplyToUnit(unitImpl)
	...
	local HAS_MOD = unitImpl:hasModifier(Id.new('g070um0267')) or unitImpl:hasModifier(Id.new('g070um0268')) or unitImpl:hasModifier(Id.new('g070um0269')) or unitImpl:hasModifier(Id.new('g070um0270'))
	if HAS_MOD then
		return false
	end
	return true
...
function getAttackSource(unit, prev)
	if unit.impl.attack1.source ~= Source.Air then
		return Source.Air
	end
...
function getAttackInitiative(unit, prev)
	local mana_criteria = math.min(1000, _getPlayerBank(unit).lifeMana)
	return svFlatEffectInitiative(unit, prev, 0.02 * mana_criteria)
end