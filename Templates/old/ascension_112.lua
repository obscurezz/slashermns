...
function getSkirmishZone(zoneId, zoneSize, race1, race2)
    local randomOrbTalisman = { 'g000ig9027', 'g001ig0178', 'g001ig0176', 'g001ig0461', 'g001ig0177', 'g000ig9112', 'g001ig0185', 'g000ig9037', 'g000ig9141', 'g000ig9128' }
...
function getZones(races)
    ...
    zones[1] = getStartingZone(0, races[1], 256)
    ...
    zones[5] = getStartingZone(4, races[2], 256)
    ...
    zones[9] = getSkirmishZone(8, 256, races[1], races[2])
...
function getConnections()
    ...
    for i = 13,14 do
        connections[i] = { from = 1, to = 2, guard = getGuard(1.02, subraceRandomizer()) }
    end

    for i = 15,16 do
        connections[i] = { from = 3, to = 2, guard = getGuard(1.02, subraceRandomizer()) }
    end

    for i = 17,18 do
        connections[i] = { from = 5, to = 6, guard = getGuard(1.02, subraceRandomizer()) }
    end

    for i = 19,20 do
        connections[i] = { from = 7, to = 6, guard = getGuard(1.02, subraceRandomizer()) }
    end
...
template = {
    name = 'ascension 1.1.2',
    description = 'ascension 1.1.2S by obscure, SMNS2, 48x48, 2 players',
    minSize = 48,
    maxSize = 48,
    maxPlayers = 2,
    roads = 45,
